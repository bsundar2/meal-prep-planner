...
### Installable as a home-screen app (PWA manifest, not a native APK)

`src/app/manifest.ts` (Next.js's special file convention — auto-linked into `<head>`, served at `/manifest.webmanifest`) plus `public/icons/*.png` make the deployed site installable from Android Chrome's "Install app" / "Add to Home screen" menu: a standalone window with no browser chrome, a real home-screen icon, indistinguishable in daily use from a native app. This is deliberately *not* a TWA-wrapped or Capacitor-wrapped standalone `.apk` — Chrome builds and signs a minimal on-device package itself from the manifest, entirely automatically, so there's no build pipeline, no signing keystore to manage, and no native project in this repo. A real standalone, distributable `.apk` file is a materially bigger undertaking (a TWA build via a tool like [Bubblewrap](https://github.com/GoogleChromeLabs/bubblewrap), which needs the Android SDK — not installable in this project's dev sandbox, whose network egress policy blocks Google's SDK-download hosts, so it would have to run in CI, e.g. GitHub Actions, with a signing keystore kept as a secret) and hasn't been built; this manifest-only approach was chosen deliberately as the one that ships today with no new infrastructure.

The three icon files were generated once via `next/og`'s `ImageResponse` (Satori-based JSX-to-PNG rendering, the same engine Next.js uses for OG-image generation) run as a standalone script, **not** as an `icon.tsx`/`apple-icon.tsx` special-file route — those generate favicon-style `<link>` tags with hashed URLs Next computes internally, which isn't something `manifest.ts`'s `icons` array can reference reliably; plain static files in `public/` with fixed paths are what a manifest needs. The icon content is a plain "MP" text monogram, not the emoji originally tried — Satori doesn't rasterize color emoji glyphs without an explicit `graphemeImages` font mapping, and without one the glyph silently renders as nothing (confirmed: the emoji version produced a blank icon, same file size for both the regular and "maskable" padding variants, which is what exposed the bug). `icon-maskable-512.png` carries extra padding for Android's adaptive-icon safe zone (maskable icons get cropped to a circle/squircle by the OS; content outside the safe area gets clipped).

No service worker is included — this app is entirely server-rendered and database-backed, so there's no meaningful "offline" experience to cache toward; every page needs a live round-trip regardless. Chrome's manual "Install app" affordance works from the manifest alone (valid `name`, `icons` including a 512×512, `start_url`, `display: standalone`, served over HTTPS) without one; only the *automatic* promotional install banner has stricter criteria that typically expect a service worker, and that's not something this app needs.
